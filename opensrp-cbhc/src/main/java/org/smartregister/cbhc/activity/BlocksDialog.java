...
//        baseUrl = baseUrl + "/rest/event/ward/block?wards=";
        baseUrl = baseUrl + "/rest/event/user/ward/block?username="+AncApplication.getInstance().getContext().allSharedPreferences().fetchRegisteredANM();
...
//        JSONObject request = new JSONObject();
        ...
        baseUrl = baseUrl + "?username="+AncApplication.getInstance().getContext().allSharedPreferences().fetchRegisteredANM();
        baseUrl = baseUrl + "&password="+AncApplication.getInstance().getContext().allSettings().fetchANMPassword();
    ...
    public void updateSetttingsTable(JSONObject jsonObject) {
        try {
          ...
          //  AncApplication.getInstance().getRepository().getWritableDatabase().insert(EventClientRepository.Table.event.name(), null, values);
            ...
            AncApplication.getInstance().getRepository().getWritableDatabase().close();
        } catch (Exception e) {
            Timber.e(e);
        }
    }
...
//        System.out.println(locationTree);
    ...
    public void getNodeName(List<FormLocation> loc){
        ...
        for(int i=0;i<loc.size();i++){
            ...
            Set<String>levels = formLocation.level;
